...
namespace xs
{
	...
	namespace archive_generator
	{
		...
		// ------------------------------------------------------------------------
		// Calculate the total amount of entries.
		size_t count_total_amount_of_entries(const std::string& root, const std::vector<std::string>& subDirs)
		{
		...
		// ------------------------------------------------------------------------
		// Calculate the total size of content entries in the specified directory.
		size_t calculate_content_entries_size(const std::string& root, const std::vector<std::string>& subDirs)
		{
			fs::path root_path = fs::path(root);
		...
		void add_content_entries(BlobBuilder* builder, const fs::path& p)
		{
			...
			for (auto& entry : fs::recursive_directory_iterator(p))
			{
				...
				ulong src_len = static_cast<ulong>(entry.file_size());
				ulong cmp_len = static_cast<ulong>(resource_pipeline::is_text_file(s_entry_path_extension) ? compressBound(src_len) : 0);
				...
				if (cmp_len != 0)
				{
					...
					// Compress the data.
					s32 cmp_status = compress((u8*)data.data(), &cmp_len, (const u8*)f.data(), src_len);
					if (cmp_status != Z_OK)
					{
					...
					resource_pipeline::ContentHeader h(s_entry_path, builder->offset() + sizeof(resource_pipeline::ContentHeader), src_len, cmp_len);

					log::info("Content Header:");
					log::info("\tPath: {}", s_entry_path);
					log::info("\tFile offset: {}", builder->offset() + sizeof(resource_pipeline::ContentHeader));
		...
		// ------------------------------------------------------------------------
		// Add content entries to the BlobBuilder.
		void add_content_entries(BlobBuilder* builder, const std::string& root, const std::vector<std::string>& subDirs)
		{
			fs::path root_path = fs::path(root);
		...
		Archive generate(const std::string& root, const std::vector<std::string>& subDirs)
		{
			size_t entry_count = count_total_amount_of_entries(root, subDirs);
			...
			total_size += calculate_content_entries_size(root, subDirs);
			...
			BlobBuilder builder = BlobBuilder(blob.get());
			...
			add_content_entries(&builder, root, subDirs);
	...
	{
		// ------------------------------------------------------------------------
		size_t write_data_to_archive(std::ofstream& stream, const char8* data, size_t size)
		{
		...
		bool write_archive(const std::string& outputPath, const archive_generator::Archive& archive)
		{
			std::ofstream ofs;
			...
			if (ofs.is_open())
			{
				size_t total_size = write_data_to_archive(ofs, reinterpret_cast<const char8*>(archive.blob->data()), archive.size * sizeof(u8));

				ofs.close();

				log::info("Written ({0} bytes) archive to disk {1}", total_size, outputPath);
		...
		{
			archive_generator::Archive archive = archive_generator::generate(root, subDirs);

			// Write the compressed archive to a file.
			return write_archive(resource_pipeline::make_archive_path(root, subDirs), archive);
		}
	}
}