...
namespace xs::fileio::internal
{
	...
	unordered_map<std::string, resource_pipeline::ContentHeader> text_content_headers;
	unordered_map<std::string, resource_pipeline::ContentHeader> binary_content_headers;
	...
	void load_game_content_headers()
	{
		...
		for (s32 i = 0; i < entries_count; ++i)
		{
			resource_pipeline::ContentHeader header;
			offset += read_from_archive(&header, game_content.data() + offset, sizeof(resource_pipeline::ContentHeader));
			...
			if (header.file_size_compressed != 0)
			{
				...
				// Offset with the content data itself so we can jump to the next ContentHeader
...
Blob fileio::read_binary_file(const string& filename)
{
	...
	if (has_binary_content && has_given_content_file)
	{
		...
		const resource_pipeline::ContentHeader& header = binary_content_headers.at(path);
		file.seekg(header.file_offset, ios::beg);
		Blob buffer(header.file_size);
		if (file.read((char8*)buffer.data(), header.file_size))
			return buffer;
	}
	else
	{
...
string fileio::read_text_file(const string& filename)
{
	...
	if (has_text_content && has_given_content_file)
	{
		...
		const resource_pipeline::ContentHeader& header = text_content_headers.at(path);

		file.seekg(header.file_offset, ios::beg);
		Blob compressed_buffer(header.file_size_compressed);
		if (file.read((char8*)compressed_buffer.data(), header.file_size_compressed))
		{
			ulong size = (ulong)header.file_size;
			string buffer(size, '\0');

			// Decompress using miniz library
			s32 dcmp_status = uncompress((u8*)buffer.data(), &size, (const u8*)(compressed_buffer.data()), (mz_ulong)(header.file_size_compressed));
			if (dcmp_status != Z_OK)
			{
