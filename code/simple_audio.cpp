...
namespace xs::simple_audio
{
	...
	void initialize()
	{
		...
		{
			log::warning("SimpleAudio already initialized");
			return;
		}
...
#else
		log::warning("SimpleAudio is only implemented for PC and Apple platforms");
#endif
	}
	...
	int load(const std::string& filename)
	{
		...
		return hash;
#else
		log::error("SimpleAudio not implemented for this platform");
		return -1;
#endif
	}
	...
	int play(int audio_id, double volume)
	{
		...
		{
			log::warning("Failed to set audio stream gain: {}", SDL_GetError());
		}
	...
	void set_volume(int channel_id, double volume)
	{
		...
		{
			log::warning("Channel {} not found", channel_id);
			return;
		}
